...
export const state = () => ({
  seoDefault: {
    image:
      "https://images.ctfassets.net/xsmgpzj6d8er/5TyiTSFZiYTicI5pOIMlfy/baffbec0099fb75bd71f6cf1e816673c/Aerial_All_Buildings_Design_District.jpg?w=2000&fm=jpg&q=80",
    title: "Design District",
    description:
      "A new permanent home for the creative industries. With purpose-designed workspaces, workshops, accessible rents and flexible leases."
  },
  navigation: {
    theme: "light"
  },
...
export const mutations = {
  ...
  // setAppMapDraggable(state, payload) {
  //   state.appMapDraggable = payload?.draggable;
  // },
  // setIsMobilePortrait(state, payload) {
  //   state.isMobilePortrait = payload;
  // }
};
...
export const getters = {
  ...
  getSpaceBySlug: state => slug => {
    return state.spaces.length > 0
      ? state.spaces.find(space => space.slug === slug)
      : null;
  },
  getBuildingBySlug: state => slug => {
    // console.log(slug)
    return state.buildings.length > 0
      ? state.buildings.find(building => building.slug === slug)
      : null;
  },
  getSpacesByBuilding: state => slug => {
    return state.spaces.length > 0
      ? state.spaces.filter(space => space.building === slug)
      : null;
  },
  getHireBySlug: state => slug => {
    return state.hire.length > 0
      ? state.hire.find(hire => hire.slug === slug)
      : null;
  },
  getStudioBySlug: state => slug => {
    return state.studios.length > 0
      ? state.studios.find(studios => studios.slug === slug)
      : null;
  },
  getStudioByBuilding: state => building => {
    return state.studios.length > 0
      ? state.studios.find(studios =>
          _.includes(_.split(studios.buildings, " "), building)
        )
      : null;
  }
};
...
export const actions = {
  ...
  // setAppMapDraggable({ commit }) {
  //   commit("setAppMapDraggable");
  // },
  // setIsMobilePortrait({ commit }) {
  //   commit("setIsMobilePortrait");
  // },
  ...
  async getJournals({ commit }) {
    ...
    if (response.items.length > 0) {
      ...
      const featuredOrderedByDate = _.orderBy(
        featured,
        ["sys.createdAt"],
        ["desc"]
      );
      const nonFeatured = _.filter(
        response.items,
        item => !item.fields.featured
      );
      const nonFeaturedOrderedByDate = _.orderBy(
        nonFeatured,
        ["sys.createdAt"],
        ["desc"]
      );
      ...
      commit("setJournals", {
        journals: featuredOrderedByDate.concat(nonFeaturedOrderedByDate)
      });
    }
  },
  async getEvents({ commit }) {
    ...
    if (response.items.length > 0) {
      ...
      commit("setEvents", {
        events: featuredOrderedByDate.concat(nonFeaturedOrderedByDate)
      });
